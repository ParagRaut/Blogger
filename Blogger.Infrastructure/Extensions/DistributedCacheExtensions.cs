...
/// </summary>
/// <remarks>
///     The overloads taking <c>TState</c> are useful when used with <c>static</c> get methods, to avoid
...
public static class DistributedCacheExtensions
{
    ...
    {
        return GetAsyncShared<int, T>(cache, key, 0, getMethod, options, cancellation);
        // use dummy state
    }

    /// <summary>
    ///     Gets a value from cache, with a caller-supplied <paramref name="getMethod" /> (sync, stateless) that is used if the
    ...
    /// <summary>
    ///     Gets a value from cache, with a caller-supplied <paramref name="getMethod" /> (async, stateful) that is used if the
    ///     value is not yet available
    /// </summary>
    public static ValueTask<T> GetAsync<TState, T>(
        ...
        CancellationToken cancellation = default)
    {
        return GetAsyncShared<TState, T>(cache, key, state, getMethod, options, cancellation);
    }

    /// <summary>
    ///     Gets a value from cache, with a caller-supplied <paramref name="getMethod" /> (sync, stateful) that is used if the
    ///     value is not yet available
    /// </summary>
    public static ValueTask<T> GetAsync<TState, T>(
        this IDistributedCache cache,
    ...
    /// </summary>
    private static ValueTask<T> GetAsyncShared<TState, T>(
    ...
    {
        Task<byte[]?> pending = cache.GetAsync(key, cancellation);
        ...
        return new ValueTask<T>(Deserialize<T>(bytes));
